...
function Ebak_Rep($tablename,$dbname,$userid,$username){
	...
	if(empty($count))
	{printerror("MustChangeOcomable","history.go(-1)");}
	for($i=0;$i<$count;$i++)
	{
...
?>";
	...
	$empire->free($sql);
	//进入下一个表
	//echo $btb[$t-1].$fun_r['OcomableBakSuccess']."<script>self.location.href='phome.php?phome=BakExe&s=0&p=0&t=$t&mypath=$mypath&stime=$stime".hReturnEcmsHashStrHref(0)."';</script>";

	echo"<meta http-equiv=\"refresh\" content=\"".$waitbaktime.";url=phome.php?phome=BakExe&s=0&p=0&t=$t&mypath=$mypath&stime=$stime&waitbaktime=$waitbaktime".hReturnEcmsHashStrHref(0)."\">".$btb[$t-1].$fun_r['OcomableBakSuccess'];
	exit();
}
...
?>";
	...
	if(empty($b))
	{
		...
		$empire->free($sql);
		//进入下一个表
		//echo $btb[$t-1].$fun_r['OcomableBakSuccess']."<script>self.location.href='phome.php?phome=BakExeT&s=0&p=0&t=$t&mypath=$mypath&stime=$stime".hReturnEcmsHashStrHref(0)."';</script>";

		echo"<meta http-equiv=\"refresh\" content=\"".$waitbaktime.";url=phome.php?phome=BakExeT&s=0&p=0&t=$t&mypath=$mypath&stime=$stime&waitbaktime=$waitbaktime".hReturnEcmsHashStrHref(0)."\">".$btb[$t-1].$fun_r['OcomableBakSuccess'];
		exit();
	}
	//进入下一组
	$p++;
