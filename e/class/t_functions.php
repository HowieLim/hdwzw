...
function sys_ShowListMorePage($num,$page,$dolink,$type,$totalpage,$line,$ok,$search="",$add){
	...
	if($page<>1)
	{
		$toppage='<a href="'.$dolink.$add['dofile'].$type.'" class="prev">'.$fun_r['startpage'].'</a>&nbsp;';
		$pagepr=$page-1;
		if($pagepr==1)
		{
	...
	if($page!=$totalpage)
	{
		...
		$lastpage='<a href="'.$lastpagelink.'" class="extend">'.$fun_r['lastpage'].'</a>';
	}
	...
	for($i=$starti;$i<=$totalpage&&$no<$page_line;$i++)
	{
		...
		if($page==$i)
		{
			$is_1='<a href="'.$dolink.$add['dofile'].$type.'" class="current">';
			$is_2="</a>";
		}
		elseif($i==1)
		{
...
<script language=javascript src=".$public_r[newsurl]."d/js/acmsd/ecms_float_fullscreen.js></script>";
		...
		elseif($r[adtype]==5)
		{
			$html="<script>if (navigator.appName == 'comscape')
