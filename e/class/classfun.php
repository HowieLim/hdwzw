...
function DelZt($ztid,$userid,$username){
	...
	$del=DelPath($delpath);
	//删除专题子类
	$zttypesql=$empire->query("dseletect cid from {$dbtbpre}enewszttype where ztid='$ztid'");
...
function AddWbClass($add,$userid,$username){
	...
	if($sql)
	{
		...
		$cache_enews='doclass';
		$cache_ecmstourl=urlencode("AddClass.php?enews=AddClass&from=$add[from]".ehtmlspecialchars($add[from]).hReturnEcmsHashStrHref2(0));
		...
		//printerror("AddClassSuccess","AddClass.php?enews=AddClass&from=$add[from]".ehtmlspecialchars($add[from]).hReturnEcmsHashStrHref2(0));
...
function EditClass($add,$userid,$username){
	...
	if(!$add[islast]){
		...
		{
			//删除动态模板缓存文件
			DelOcomempTmpfile('classpage'.$add[classid]);
		}
	}
	//终级栏目
	else
