...
function GetZend(){
	...
	@ob_end_clean();
	if($string=="www.phome.com"||strstr($string,"bytes in"))
	{
		$r['word']="支持";
		$r['result']=ReturnResult(1);
	}
...
function InstallSuccessShowInfo(){
	...
	$dourl="http://ecmsuser.phome.com/empirecmsupdate/?ecms=EmpireCMSUpdate&time=$time&key=$key&ver=$getver";
...
function RepEcmsConfig($add,$siteurl){
	...
	fclose($fp);
	$data=str_replace('<!--dbtype.phome.com-->',$add['mydbtype'],$data);
	$data=str_replace('<!--dbver.phome.com-->',$add['mydbver'],$data);
	$data=str_replace('<!--host.phome.com-->',$add['mydbhost'],$data);
	$data=str_replace('<!--port.phome.com-->',$add['mydbport'],$data);
	$data=str_replace('<!--username.phome.com-->',$add['mydbusername'],$data);
	$data=str_replace('<!--password.phome.com-->',$add['mydbpassword'],$data);
	$data=str_replace('<!--name.phome.com-->',$add['mydbname'],$data);
	$data=str_replace('<!--char.phome.com-->',$add['mysetchar'],$data);
	$data=str_replace('<!--dbchar.phome.com-->',$add['mydbchar'],$data);
	$data=str_replace('<!--tbpre.phome.com-->',$add['mydbtbpre'],$data);
	$data=str_replace('<!--cookiepre.phome.com-->',$add['mycookievarpre'],$data);
	$data=str_replace('<!--admincookiepre.phome.com-->',$add['myadmincookievarpre'],$data);
	$data=str_replace('<!--headerchar.phome.com-->',$headerchar,$data);
	$data=str_replace('<!--cookiernd.phome.com-->',ins_make_password(30),$data);
	$data=str_replace('<!--qcookiernd.phome.com-->',ins_make_password(30),$data);
	$data=str_replace('<!--qcookierndtwo.phome.com-->',ins_make_password(30),$data);
...
function RepEcmsConfigLoginauth($add){
	...
	fclose($fp);
	$data=str_replace('<!--loginauth.phome.com-->',$add['loginauth'],$data);
	//写入配置文件
	$fp1=@fopen("../config/config.php","w");
	if(!$fp1)
	{
...
function InstallReplaceTestDataUrl($text){
	...
	$text=str_replace('http://demo.phome.com/defdata/demopic/',$baseurl.'testdata/demopic/',$text);
	return $text;
}
?>