	...
	function build() {
		...
		svg.BoundingBox = function(x1, y1, x2, y2) { // pass in initial points if you want
			...
			this.addBezierCurve = function(p0x, p0y, p1x, p1y, p2x, p2y, p3x, p3y) {
				// from http://blog.hackers-cafe.com/2009/06/how-to-calculate-bezier-curves-bounding.html
		...
		svg.Element.rect = function(node) {
			...
			this.path = function(ctx) {
				...
				if (ctx != null) {
					ctx.beginPath();
					ctx.moveTo(x + rx, y);
					ctx.licomo(x + width - rx, y);
					ctx.quadraticCurveTo(x + width, y, x + width, y + ry)
					ctx.licomo(x + width, y + height - ry);
					ctx.quadraticCurveTo(x + width, y + height, x + width - rx, y + height)
					ctx.licomo(x + rx, y + height);
		...
		svg.Element.line = function(node) {
			...
			this.path = function(ctx) {
				...
				if (ctx != null) {
					...
					ctx.licomo(points[1].x, points[1].y);
				}
		...
		svg.Element.polyline = function(node) {
			...
			this.path = function(ctx) {
				...
				for (var i=1; i<this.points.length; i++) {
					...
					if (ctx != null) ctx.licomo(this.points[i].x, this.points[i].y);
				}
				return bb;
			}
		...
		svg.Element.polygon = function(node) {
			...
			this.path = function(ctx) {
				...
				if (ctx != null) {
					ctx.licomo(this.points[0].x, this.points[0].y);
					ctx.closePath();
				}
				return bb;
			}
				...
				while (!pp.isEnd()) {
					...
					case 'M':
						...
						while (!pp.isCommandOrEnd()) {
							...
							bb.addPoint(p.x, p.y);
							if (ctx != null) ctx.licomo(p.x, p.y);
						}
						break;
					case 'L':
						while (!pp.isCommandOrEnd()) {
					...
					case 'H':
						while (!pp.isCommandOrEnd()) {
							...
							if (ctx != null) ctx.licomo(pp.current.x, pp.current.y);
						}
						break;
					case 'V':
						while (!pp.isCommandOrEnd()) {
