...
//验证验证码
function CheckOnlinePass($onlicomime,$onlinepass){
	global $movtime,$public_r;
	if($onlicomime+$movtime<time()||$onlicomime>time())
	{
		exit();
	}
	$onlinep=$public_r[downpass]."qweirtydui4opttt.,mvcfvxzzf3dsfm,.dsa";
	$cpass=md5($onlinep.$onlicomime);
	if($onlinepass<>$cpass)
	{
		exit();
	}
...
//--------取得软件地址
function GetSofturl($classid,$id,$pathid,$p,$pass,$onlicomime,$onlinepass){
	...
	{exit();}
	//验证验证码
	CheckOnlinePass($onlicomime,$onlinepass);
	//表不存在
	if(empty($class_r[$classid][tbname]))
	{exit();}
	$mid=$class_r[$classid][modid];
